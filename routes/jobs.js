...
router.post("/:id/apply", async function(req, res, next) {
  try {
    ...
    const app = await Job.apply(req.params.id, req.body.username, state);
    return res.json(app);
  }
...
router.patch("/:id/withdraw", authRequired, async function(req,res,next){
  try{
    await Job.withdraw(req.params.id);
    return res.json({message:'withdrawed'})
  }catch(err){
    return next(err);
  }
})
